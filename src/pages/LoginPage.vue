          ...
          <v-card class="login-card" elevation="8">
            ...
            <v-card-text>
              <v-form @submit.prevent="login" ref="form" lazy-validation>
                ...
                <v-btn color="primary" block class="mt-4" type="submit">Login</v-btn>
              </v-form>
              ...
              <v-alert
                ...
                elevation="2"
              >
                Invalid email, password, or CAPTCHA. Please try again.
              </v-alert>
            </v-card-text>
          </v-card>
...
//   };
  ...
  // ✅ Login Handler
  const login = () => {
    // if (form.value.validate() && captchaVerified.value) {
    if (form.value.validate()) {
      if (email.value === 'admin@mulachannels.com' && password.value === 'password123') {
